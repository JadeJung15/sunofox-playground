    ...
    app.innerHTML = `
        <div class="category-page fade-in">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 1.5rem;">
                ${categories.map(cat => `
                    <div class="bento-item" onclick="location.hash='#${cat.id}'" style="background: ${cat.bg}; text-align: center;">
                        ...
                        <p style="font-size: 0.95rem; color: #64748b; font-weight: 600; margin-bottom: 2.2rem;">${cat.desc}</p>
                        <span style="font-size: 0.9rem; font-weight: 800; color: ${cat.text}; background: #fff; padding: 10px 22px; border-radius: 50px; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">리포트 보기 →</span>
                    </div>
                `).join('')}
            </div>
        </div>
        ...
        app.innerHTML = `
            <div class="dashboard-container fade-in">
                ...
                <div style="margin-bottom: 2.5rem; padding: 0 0.5rem;">
                    <p style="font-size: 0.85rem; font-weight: 800; color: var(--accent-color); letter-spacing: 0.1em; margin-bottom: 4px;">DASHBOARD</p>
                    <h2 style="font-size: 1.8rem; font-weight: 900; color: #1e293b; letter-spacing: -0.02em; margin-bottom: 1.5rem;">반가워요, ${userName}님! 👋</h2>
                    <div style="position: relative; width: 100%; max-width: 600px;">
                        <input type="text" id="home-search" placeholder="궁금한 분석 키워드를 검색해보세요" 
                               style="width: 100%; padding: 1.1rem 1.5rem 1.1rem 3.5rem; border-radius: 20px; border: 2px solid #e2e8f0; background: #fff; font-size: 1rem; font-weight: 600; outline: none; transition: 0.3s; box-shadow: var(--shadow-sm);">
                        <span style="position: absolute; left: 1.2rem; top: 50%; transform: translateY(-50%); font-size: 1.4rem; opacity: 0.4;">🔍</span>
                    </div>
                </div>

                <!-- 2. 퀵 메뉴 (대칭 배치) -->
                <div style="display: flex; justify-content: center; gap: 1rem; width: 100%; margin-bottom: 3rem; overflow-x: auto; padding: 0.5rem; scrollbar-width: none; -webkit-overflow-scrolling: touch;">
                    ${[
                        ...
                        <div onclick="location.hash='#${item.id}'" style="flex: 1; min-width: 75px; max-width: 90px; text-align: center; cursor: pointer; transition: transform 0.2s;">
                            ...
                            <span style="font-size: 0.8rem; font-weight: 800; color: #1e293b;">${item.label}</span>
                        </div>
                    `).join('')}
                </div>
                ...
                <div style="display: flex; justify-content: center; width: 100%; margin-bottom: 4rem;">
                    <div class="hero-section" style="width: 100%; max-width: 450px; aspect-ratio: 1 / 1; background: linear-gradient(135deg, #1e1b4b 0%, #312e81 100%); border-radius: 48px; box-shadow: var(--shadow-premium); padding: 2rem; color: #fff; text-align: center;">
                        <div style="position: absolute; inset: 0; background: radial-gradient(circle at 50% 0%, var(--accent-color) 0%, transparent 70%); opacity: 0.4;"></div>
                        <div style="position: relative; z-index: 2;">
                            <div style="background: rgba(255,255,255,0.12); backdrop-filter: blur(10px); padding: 0.8rem 1.5rem; border-radius: 20px; border: 1px solid rgba(255,255,255,0.2); margin-bottom: 2rem; font-weight: 700; font-size: clamp(0.85rem, 3.5vw, 1rem); display: inline-block; max-width: 90%;">
                                "${randomAdvice}"
                            </div>
                            <h1 style="font-size: clamp(2rem, 9vw, 2.6rem); font-weight: 900; line-height: 1.2; margin-bottom: 1.5rem; letter-spacing: -0.04em;">진짜 당신을<br>확인할 시간</h1>
                            <button class="btn-primary" style="padding: 1.1rem 3rem; font-size: clamp(1rem, 4vw, 1.15rem); border-radius: 24px; background: #fff; color: #1e1b4b; font-weight: 900; border: none; box-shadow: 0 15px 30px rgba(255,255,255,0.2);" onclick="location.hash='#7check'">시작하기 ➔</button>
                        </div>
                    </div>
                </div>
                ...
                <div style="width: 100%; margin-bottom: 4rem;">
                    <div class="category-header">
                        <h3 style="font-size: 1.4rem; font-weight: 900; color: #1e293b;">🔥 지금 핫한 분석</h3>
                    ...
                    <div id="test-list-grid" class="test-grid">
                ...
                <div class="bento-grid">
                    <div class="bento-item" onclick="location.hash='#arcade'" style="background: linear-gradient(135deg, #ecfdf5 0%, #d1fae5 100%); border: 1px solid #a7f3d0; min-height: 200px; display: flex; flex-direction: column; justify-content: center;">
                        <h4 style="font-size: 1.6rem; font-weight: 900; color: #064e3b; margin-bottom: 0.6rem;">세븐 오락실</h4>
                        <p style="font-size: 1rem; font-weight: 600; color: #059669; margin-bottom: 1.8rem;">포인트를 모아 펫을 입양하세요.</p>
                        <button style="background: #059669; color: #fff; border: none; padding: 10px 22px; border-radius: 14px; font-weight: 900; font-size: 0.9rem; width: fit-content;">입장하기</button>
                    ...
                    <div class="bento-item" onclick="location.hash='#ranking'" style="min-height: 200px; display: flex; flex-direction: column; justify-content: center;">
                        <h4 style="font-size: 1.6rem; font-weight: 900; color: #1e293b; margin-bottom: 0.6rem;">명예의 전당</h4>
                        <p style="font-size: 1rem; font-weight: 600; color: #64748b; margin-bottom: 1.8rem;">최고의 분석가 순위 확인</p>
                        <button style="background: #1e293b; color: #fff; border: none; padding: 10px 22px; border-radius: 14px; font-weight: 900; font-size: 0.9rem; width: fit-content;">랭킹보기</button>
    ...
    <div class="test-card" onclick="location.hash='#test/${t.id}'">
        <div class="test-card-info">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                <span class="test-card-cat">${t.category}</span>
                <span style="font-size: 0.75rem; font-weight: 800; color: #64748b;">🔥 ${formatEngUnit(playCount)} Play</span>
            </div>
            <h3 style="word-break: keep-all; margin-bottom: 0.6rem;">${t.title}</h3>
            <p style="display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; word-break: keep-all; margin-bottom: 1.5rem;">${t.desc}</p>
