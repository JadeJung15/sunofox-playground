import { UserState, updateUI, updateProfileCache } from '../auth.js?v=8.0.0';
...
export function renderProfile() {
    ...
    app.querySelectorAll('.btn-pet-equip').forEach(btn => {
        btn.onclick = async () => {
            ...
            const { changePet } = await import('../auth.js?v=8.0.0');
    ...
    app.querySelectorAll('.btn-pet-buy').forEach(btn => {
        btn.onclick = async () => {
            ...
            const { usePoints, updateUI } = await import('../auth.js?v=8.0.0');
    ...
    app.querySelectorAll('.color-btn').forEach(btn => {
        btn.onclick = async () => {
            ...
            const { changeNameColor } = await import('../auth.js?v=8.0.0');
    ...
    app.querySelectorAll('.emoji-btn').forEach(btn => {
        btn.onclick = async () => {
            ...
            const { handleEmojiExchange } = await import('../auth.js?v=8.0.0');
    ...
    if (nickSaveBtn) {
        nickSaveBtn.onclick = async () => {
            const { changeNickname } = await import('../auth.js?v=8.0.0');
