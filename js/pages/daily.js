import { fetchDailyTest, fetchDailyTests, getDailyPath, requestDailyResult } from '../services/daily.js?v=8.5.0';
import { saveAsStoryImage } from '../share.js?v=8.5.0';
...
function renderLoading(message = '불러오는 중...') {
  ...
  app.innerHTML = `
    <div class="fade-in" style="max-width:1080px; margin:0 auto; padding:1.5rem 1rem 5rem;">
      <div style="background:#fff; border:1px solid #e2e8f0; border-radius:28px; padding:2.8rem 1.2rem; text-align:center; color:#475569; font-weight:800;">
        ${message}
      </div>
    </div>
  `;
...
function renderError(message) {
  ...
  app.innerHTML = `
    <div class="fade-in" style="max-width:1080px; margin:0 auto; padding:1.5rem 1rem 5rem;">
      <div style="background:#fff; border:1px solid #fecaca; border-radius:28px; padding:2rem 1.2rem; text-align:center;">
...
function renderResultSection(test, payload) {
  ...
  return `
    <section style="margin-top:1.15rem;">
      <div style="background:#fff; border:1px solid ${result.accent}22; border-radius:30px; padding:1.15rem; box-shadow:0 20px 40px rgba(15,23,42,0.06);">
        ...
        <div id="daily-capture-card" style="background:linear-gradient(145deg,#ffffff 0%, ${result.accent}12 100%); border:1px solid ${result.accent}22; border-radius:28px; padding:1.5rem 1.2rem; margin-bottom:1rem;">
          <div style="font-size:0.72rem; letter-spacing:0.12em; font-weight:900; color:${result.accent}; margin-bottom:0.5rem;">공유용 카드</div>
          <div style="font-size:2.35rem; font-weight:950; color:${result.accent}; line-height:1; letter-spacing:-0.05em; margin-bottom:0.6rem;">${result.headline}</div>
          <div style="font-size:0.95rem; font-weight:900; color:#0f172a; margin-bottom:0.45rem;">${result.subline}</div>
          <div style="font-size:1.02rem; font-weight:800; color:#1e293b; line-height:1.58; word-break:keep-all;">${result.comment}</div>
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(170px,1fr)); gap:0.7rem; margin-bottom:0.55rem;">
          <button id="daily-copy-btn" style="border:none; border-radius:16px; min-height:58px; background:${result.accent}; color:#fff; font-weight:900; cursor:pointer;">결과 복사</button>
          <button id="daily-save-btn" style="border:1px solid ${result.accent}; border-radius:16px; min-height:58px; background:#fff; color:${result.accent}; font-weight:900; cursor:pointer;">이미지 저장</button>
          <button onclick="window.goToDaily('/daily')" style="border:none; border-radius:16px; min-height:58px; background:#0f172a; color:#fff; font-weight:900; cursor:pointer;">다른 테스트</button>
        </div>
...
export async function renderDailyList() {
  ...
  try {
    ...
    app.innerHTML = `
      <div class="fade-in" style="max-width:1120px; margin:0 auto; padding:1.35rem 1rem 5rem;">
        <section style="margin-bottom:1.2rem; border-radius:34px; padding:1.25rem; background:linear-gradient(145deg,#f8fafc 0%,#dbeafe 42%,#ecfeff 100%); border:1px solid #dbeafe; box-shadow:0 22px 48px rgba(15,23,42,0.08);">
          <div style="display:grid; grid-template-columns:minmax(0,1.15fr) minmax(260px,0.85fr); gap:1rem; align-items:stretch;">
            <div style="background:linear-gradient(135deg,#111827 0%,#1d4ed8 55%,#0ea5e9 100%); border-radius:28px; padding:1.5rem; color:#fff;">
            ...
            <div style="display:grid; gap:0.8rem;">
              <div style="background:#fff; border-radius:24px; border:1px solid #dbeafe; padding:1rem; box-shadow:0 14px 28px rgba(15,23,42,0.04);">
              ...
              <div style="display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:0.7rem;">
                <div style="background:#fff; border-radius:22px; border:1px solid #e2e8f0; padding:0.95rem;">
        ...
        <section>
          <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(240px,1fr)); gap:0.9rem;">
            ${tests.map((test, index) => `
              <article style="background:#fff; border-radius:24px; border:1px solid #e2e8f0; padding:1rem; box-shadow:0 14px 30px rgba(15,23,42,0.05); display:flex; flex-direction:column; gap:0.85rem;">
                ...
                <div style="margin-top:auto; display:flex; justify-content:space-between; align-items:center; gap:0.7rem;">
                  ...
                  <button onclick="window.goToDaily('${getDailyPath(test.slug)}')" style="border:none; border-radius:14px; padding:0.75rem 0.95rem; background:linear-gradient(135deg,#2563eb,#0ea5e9); color:#fff; font-weight:900; cursor:pointer;">시작하기</button>
                </div>
              </article>
            `).join('')}
          </div>
...
export async function renderDailyDetail(slug) {
  ...
  try {
    ...
    const app = document.getElementById('app');
    app.innerHTML = `
      <div class="fade-in" style="max-width:860px; margin:0 auto; padding:1.4rem 1rem 5rem;">
        <section style="background:linear-gradient(145deg,#ffffff 0%,#f8fafc 52%,#eff6ff 100%); border:1px solid #dbeafe; border-radius:34px; padding:1.2rem; box-shadow:0 22px 44px rgba(15,23,42,0.06);">
          <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:1rem; flex-wrap:wrap; margin-bottom:0.95rem;">
            <div>
            ...
            <button onclick="window.goToDaily('/daily')" style="border:none; border-radius:14px; padding:0.75rem 0.95rem; background:#fff; color:#0f172a; font-weight:900; cursor:pointer; border:1px solid #cbd5e1;">목록 보기</button>
          </div>

          <div style="background:#0f172a; color:#fff; border-radius:28px; padding:1.4rem 1.2rem; margin-bottom:1rem;">
          ...
          <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(150px,1fr)); gap:0.7rem; margin-bottom:1rem;">
            <div style="background:#fff; border-radius:20px; border:1px solid #e2e8f0; padding:0.95rem;">
          ...
          <button id="daily-run-btn" style="width:100%; border:none; border-radius:18px; padding:1rem 1.1rem; background:linear-gradient(135deg,#2563eb 0%,#0ea5e9 100%); color:#fff; font-size:1rem; font-weight:950; cursor:pointer; box-shadow:0 16px 28px rgba(37,99,235,0.25);">결과 보기</button>
